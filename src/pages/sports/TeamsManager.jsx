...
import { MdAdd, MdEdit, MdDelete, MdClose, MdCheck, MdSearch, MdLinkOff, MdLink } from 'react-icons/md'
...
export default function TeamsManager({ activeSite, sport }) {
  ...
  // Every team for this sport, regardless of which competition/season it's
  // currently on. Used to add a team to this league+season's roster without
  // recreating it — promotions, relegations, or a fresh season rollover.
  ...
  const fetchSeasonsList = async () => {
    if (!leagueId) { setSeasonsList([]); return }
    try {
      const res = await getAPI(activeSite).get(`/leagues/${leagueId}/roster-seasons`)
      setSeasonsList(res.data.data || [])
    } catch (e) { /* non-critical */ }
  }

  const fetchRoster = async () => {
    if (!leagueId || !season) { setRoster([]); setLoading(false); return }
    setLoading(true)
    try {
      const res = await getAPI(activeSite).get(`/leagues/${leagueId}/roster?season=${season}`)
  ...
  const fetchAllTeams = async () => {
    try {
      const res = await getAPI(activeSite).get(`/teams?sport=${sport}`)
      setAllTeams(res.data.data || [])
    } catch (e) { toast.error('Failed to load teams') }
  }

  useEffect(() => { fetchRoster(); fetchSeasonsList() }, [activeSite, leagueId, season])
  useEffect(() => { fetchAllTeams() }, [activeSite, sport])
  ...
  const refreshAll = () => { fetchRoster(); fetchSeasonsList(); fetchAllTeams() }
